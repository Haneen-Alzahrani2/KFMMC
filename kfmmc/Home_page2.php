...
<body>
  ...
  <!-- ربط قاعدة البيانات , في تسجيل الدخول -->
<?php
if(isset($_POST['add'])){
    ...
    $in=mysqli_query($con,"INSERT INTO Kfmmc"('EMPNUM','NAME','DEPARTMENT') );

    $passhash = $data ['password'];
    if(password_verify($password,$passhash)){ //التحقق من الباسورد
      $_SESSION['user'] = [
        "FullName" => $data ['Full_Name1'],
        "EmployeeNumber" => $data ['Emp_number'],
        "Department" => $data ['Department']
      ];

      $_SESSION['id'] = $id ;
      header('profile_personal.php');
    }
    else {
